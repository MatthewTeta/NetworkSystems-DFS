 ...
 */
ftp_err_t ftp_send_data(connection_t *connection, FILE *infp);
 ...
 * indicating success.
 */
ftp_err_t ftp_recv_data(connection_t *connection, FILE *outfd);
 ...
 */
ftp_err_t ftp_send_chunk(connection_t *connection, ftp_cmd_t cmd, const char *arg, ssize_t arglen);

/**
 * @brief Recieve a single command packet, useful for establishing a link (ACK)
 */
ftp_err_t ftp_recv_chunk(connection_t *connection, ftp_chunk_t *ret, int timeout, int send_ack,
